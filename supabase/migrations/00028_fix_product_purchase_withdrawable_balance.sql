-- Fix purchase_product to ensure withdrawable_balance never exceeds balance
...
BEGIN
  ...
  -- Deduct from balance and ensure withdrawable_balance doesn't exceed new balance
  UPDATE profiles
  SET balance = balance - v_product.price,
      withdrawable_balance = LEAST(withdrawable_balance, balance - v_product.price)
  WHERE id = p_user_id
...
-- Fix purchase_vip_product to ensure withdrawable_balance never exceeds balance
...
BEGIN
  ...
  -- Deduct price, add earnings, and ensure withdrawable_balance doesn't exceed new balance
  UPDATE profiles
  SET balance = balance - v_product.price + v_product.earnings,
      ...
      withdrawable_balance = LEAST(
        withdrawable_balance + v_product.earnings,
        balance - v_product.price + v_product.earnings
      )
  WHERE id = p_user_id
  RETURNING balance INTO v_new_balance;
