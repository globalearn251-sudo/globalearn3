-- Update purchase_product to only allow purchases from withdrawable_balance
-- Deduct from BOTH balance and withdrawable_balance equally
...
DECLARE
  ...
  v_withdrawable_balance DECIMAL(12, 2);
...
BEGIN
  ...
  INTO v_user_balance, v_withdrawable_balance
  FROM profiles 
  WHERE id = p_user_id;
  
  -- Check if user has enough withdrawable balance (can only buy with earned money)
  IF v_withdrawable_balance < v_product.price THEN
    RAISE EXCEPTION 'Insufficient withdrawable balance. You can only purchase products with your earned balance.';
  END IF;

  -- Deduct from BOTH balance and withdrawable_balance
  UPDATE profiles
...
-- Update purchase_vip_product to only allow purchases from withdrawable_balance
-- Deduct from BOTH balance and withdrawable_balance, then add earnings
...
BEGIN
  ...
  -- Check if user has enough withdrawable balance (can only buy with earned money)
  IF v_withdrawable_balance < v_product.price THEN
    RAISE EXCEPTION 'Insufficient withdrawable balance. You can only purchase VIP products with your earned balance.';
  END IF;

  -- Deduct from BOTH balance and withdrawable_balance, then add earnings to both
  UPDATE profiles
  SET balance = balance - v_product.price + v_product.earnings,
      withdrawable_balance = withdrawable_balance - v_product.price + v_product.earnings,
